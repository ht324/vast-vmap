...
var VastVmap = (function () {
	...
	/** @const **/
	var VAST_LINEAR_TRACKING_POINT = {
		START: 'start',
	...
	var VAST_ERROR_CODES = {
		...
		ASSET_NOT_FOUND: {
			code: 1007,
			message: 'No assets were found in the VAST ad response.',
			reportType: 'noAd'
		},
		EMPTY_RESPONSE: {
			code: 1009,
		...
		UNKNOWN_AD_RESPONSE: {
			code: 1010,
			message: 'The ad response was not understood and cannot be parsed.',
			reportType: 'noAd'
		},
		VAST_SCHEMA_ERROR: {
			code: 101,
		...
		VAST_VERSION_ERROR: {
			code: 102,
			message: 'VAST version of response not supported.'
		},
		TRAFFICKING_ERROR: {
			code: 200,
			message: 'Trafficking error. Video player received an Ad type that it was not expecting and/or cannot display.'
		},
		PLAYER_DIFFERENT_LINEARITY: {
			code: 201,
			message: 'Video player expecting different linearity.'
		},
		PLAYER_DIFFERENT_DURATION: {
			code: 202,
			message: 'Video player expecting different duration.'
		},
		PLAYER_DIFFERENT_SIZE: {
			code: 203,
			message: 'Video player expecting different size.'
		},
		...
		TOO_MANY_REDIRECTS: {
			code: 302,
			message: 'Wrapper limit reached, as defined by the video player.' +
				' Too many Wrapper responses have been received with no InLine response.'
		},
		NO_ADS_AFTER_WRAPPER: {
			code: 303,
		...
		ASSET_MISMATCH: {
			code: 403,
			message: 'Couldn’t find MediaFile that is supported by this video player,' +
				' based on the attributes of the MediaFile element.'
		},
		DISPLAY_MEDIA_FILE_PROBLEM: {
			code: 405,
			message: 'Problem displaying MediaFile.' +
				'Video player found a MediaFile with supported type but couldn’t display it.' +
				'MediaFile may include: unsupported codecs, different MIME type than MediaFile@type,' +
				' unsupported delivery method, etc.'
		},
		GENERAL_NONLINEAR_ADS_ERROR: {
			code: 500,
			message: 'General NonLinearAds error.'
		},
		NONLINEAR_DIMENSIONS_DISPLAY_AREA_ERROR: {
			code: 501,
		...
		UNABLE_FETCH_NONLINEAR_RESOURCE: {
			code: 502,
			message: 'Unable to fetch NonLinearAds/NonLinear resource.'
		},
		NO_SUPPORT_NONLINEAR_FILE: {
			code: 503,
			message: 'Couldn’t find NonLinear resource with supported type.'
		},
		GENERAL_COMPANION_ERROR: {
			code: 600,
			message: 'General CompanionAds error.'
		},
		COMPANION_DIMENSIONS_DISPLAY_AREA_ERROR: {
			code: 601,
			message: 'Unable to display Companion because creative dimensions do not fit within Companion display area (i.e., no available space).'
		},
		UNABLE_DISPLAY_COMPANION: {
			code: 602,
			message: 'Unable to display Required Companion.'
		},
		UNABLE_FETCH_COMPANION_RESOURCE: {
			code: 603,
			message: 'Unable to fetch CompanionAds/Companion resource.'
		},
		NO_SUPPORT_COMPANION_FILE: {
			code: 604,
			message: 'Couldn’t find Companion resource with supported type.'
		},
		UNDEFINED_ERROR: {
			code: 900,
	 ...
	 */
	function timecodeToString(time) {
	 ...
	 */
	function timecodeFromString(time) {
		if (time.indexOf(':') === -1) {
			return time;
		}
	...
	function attributesMap(attributes) {
		...
		for (var i = 0, ln = attributes.length; i < ln; i++) {
			m[attributes[i].name] = attributes[i].value;
		}
		return m;
	}
	 ...
	 */
	function requestAd(endpoint, onFetched, onError){
		return queryVAST(endpoint, onFetched, onError);
	}
	...
	function VASTAds(root, onAdsAvailable, onError, parentAd) {
		...
		if (adElements.length === 0) {
			console.error(parentAd, "adElements.length == 0");
			onError();
			return;
		}
	...
	function VASTAd(vast, root, parentAd, onAdAvailable) {
		...
		for (i = 0; i < creatives.length; i++) {
			...
			var creativeAttributes = attributesMap(creative.attributes);
			creative = creative.firstChild;
			...
			switch (creative.tagName) {
				case "Linear":
					n = new VASTLinear(this, creative, creativeAttributes);
					if (this.linear) {
				...
				case "CompanionAds":
					...
					case "NonLinearAds":
						...
						for (var j = 0; j < items.length; j++) {
							n = new Cls(this, items.item(j), creativeAttributes);
	...
	VASTAd.prototype.getAdId = function () {
		return this.id;
	};
	 ...
	 * @constructor
	 */
	function VASTCreative(ad, root, creativeAttributes) {
		this.root = root;
	...
	VASTCreative.prototype.track = function (ev, position, asset) {
		this.tracking.track(ev, {
			"CONTENTPLAYHEAD": timecodeToString(position),
			"ASSETURI": asset
		});
	};
	...
	VASTCreative.prototype.attribute = function (name, nothing) {
		...
		switch (name) {
			...
			case 'minSuggestedDuration':
				attr = timecodeFromString(attr);
		}
		return attr;
	};

	VASTCreative.prototype.getCreativeId = function () {
		return this.id;
	};
	 ...
	 */
	function VASTLinear(ad, root, creativeAttributes) {
		VASTCreative.call(this, ad, root, creativeAttributes);
		this.mediaFiles = [];
		this.clickThrough = null;
		this.duration = null;
		...
		if (d.length) {
			this.duration = timecodeFromString(d.item(0).textContent.replace(/\s/g, ""));
		}
		...
		for (i = 0; i < medias.length; i++) {
			var m = medias.item(i);
			var mf = attributesMap(m.attributes);
		...
		for (var i = 0; i < events.length; i++) {
			...
			switch (events[i]["event"]) {
				...
				case "skip":
					...
					if (offset.indexOf('%') === -1) {
						point["offset"] = timecodeFromString(offset);
						if (duration) {
	 ...
	 * @param {Element} root The root node of this creative in the VAST XML
	 */
	function VASTStatic(ad, root, creativeAttributes) {
		VASTCreative.call(this, ad, root, creativeAttributes);
		this.resources = {
			"iframe": null,
			"html": null,
	 ...
	 * @param {Element} root Root node of creative
	 */
	function VASTCompanion(ad, root, creativeAttributes) {
		VASTStatic.call(this, ad, root, creativeAttributes);
		this.altText = "";
	 ...
	 * @param {Element} root Root node of creative
	 */
	function VASTNonLinear(ad, root, creativeAttributes) {
		VASTStatic.call(this, ad, root, creativeAttributes);
	...
	return {
		VAST_LINEAR_TRACKING_POINT: VAST_LINEAR_TRACKING_POINT,
		VAST_ERROR_CODES: VAST_ERROR_CODES,
